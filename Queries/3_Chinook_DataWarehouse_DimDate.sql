...
DECLARE @StartDate DATETIME = (SELECT MinDate FROM ChinookStaging.dbo.Dates); --Starting value of Date Range
DECLARE @EndDate DATETIME = (SELECT MaxDate FROM ChinookStaging.dbo.Dates); --End Value of Date Range

DROP TABLE IF EXISTS [ChinookDW].[dbo].[DimDate]
...
alter table [ChinookDW].[dbo].DimDate
add constraint date_PK   primary key (DateKey);

alter table [ChinookDW].[dbo].[factSales]  add constraint fact_date_fk
foreign key (DateKey) references  [ChinookDW].[dbo].DimDate(DateKey);
